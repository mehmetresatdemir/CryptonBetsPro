...
// Game card component
const CasinoGameCard = React.memo(({ game, onClick }: { 
  ...
  const isLiveGame = game.type?.toLowerCase().includes('live') || 
                     game.name.toLowerCase().includes('live') ||
                     game.provider?.toLowerCase().includes('evolution');
  
  return (
    <div 
      onClick={() => onClick(game)}
      className="group relative bg-gradient-to-b from-gray-900 via-black to-red-900/10 rounded-xl overflow-hidden border border-red-500/20 hover:border-red-400/80 transition-all duration-300 cursor-pointer hover:scale-105 hover:shadow-2xl hover:shadow-red-500/30"
    >
      {/* Game Image */}
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        {imageError && (
          <div className="w-full h-full bg-gradient-to-br from-gray-700 to-gray-800 flex items-center justify-center">
            <div className="text-center text-gray-400">
              <Dices className="w-8 h-8 mx-auto mb-2" />
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
          <Button 
            size="sm" 
            className="bg-red-500 hover:bg-red-600 text-white font-bold px-6 py-2 rounded-full shadow-lg transform scale-90 group-hover:scale-100 transition-transform duration-300"
          >
        ...
        {isLiveGame && (
          <div className="absolute top-2 right-2">
            <Badge variant="outline" className="bg-red-500/20 border-red-500 text-red-400 text-xs px-2 py-1 animate-pulse">
              🔴 LIVE
            </Badge>
          </div>
        )}
      ...
      {/* Game information */}
      <div className="p-3">
        <h3 className="font-semibold text-white text-sm mb-1 line-clamp-1 group-hover:text-red-400 transition-colors">
          {game.name}
        </h3>
        <div className="flex items-center justify-between text-xs text-gray-400">
          ...
          <div className="flex items-center space-x-1">
            ...
            <span className="text-red-400">4.8</span>
          </div>
        </div>
      </div>
    </div>
...
const CasinoPage: React.FC = () => {
  ...
  const [gameCategory, setGameCategory] = useState<'all' | 'live' | 'table' | 'roulette' | 'blackjack' | 'baccarat' | 'favorites'>('all');
  ...
  const {
    ...
    queryKey: ['casino-games-all'],
    queryFn: async () => {
      console.log(`🎰 Loading casino games...`);
      ...
      try {
        ...
        console.log(`🎰 Casino games loaded: ${data.games?.length || 0}`);
        
        // Filter for casino games (live casino, table games, etc.)
        const casinoGames = data.games?.filter((game: any) => {
          const gameName = game.name.toLowerCase();
          const gameProvider = game.provider?.toLowerCase() || '';
          const gameType = game.type?.toLowerCase() || '';
          
          // Casino game keywords
          const casinoKeywords = ['roulette', 'blackjack', 'baccarat', 'poker', 'live', 'casino', 'table', 'wheel', 'card'];
          const casinoProviders = ['evolution', 'ezugi', 'pragmatic live', 'netent live', 'authentic gaming'];
          
          return casinoKeywords.some(keyword => 
            gameName.includes(keyword) || 
            gameType.includes(keyword)
          ) || casinoProviders.some(provider => gameProvider.includes(provider));
        }) || [];
        
        return {
          items: casinoGames,
          _meta: { totalCount: casinoGames.length },
          providers: [],
          categories: {},
          stats: {}
        };
      } catch (error) {
        console.error('❌ Casino games loading error:', error);
        throw error;
      }
    },
  ...
  const games = casinoData?.items || [];
  const totalCount = casinoData?._meta?.totalCount || 0;
  
  // Providers from games
  ...
  const filteredAndSortedGames = useMemo(() => {
    let filtered = games.filter((game: any) => {
      ...
      const matchesCategory = (() => {
        ...
        switch (gameCategory) {
          case 'live':
            return gameName.includes('live') || gameType.includes('live');
          case 'table':
            return gameName.includes('table') || ['roulette', 'blackjack', 'baccarat', 'poker'].some(t => gameName.includes(t));
          case 'roulette':
            return gameName.includes('roulette');
          case 'blackjack':
            return gameName.includes('blackjack');
          case 'baccarat':
            return gameName.includes('baccarat');
          default:
            return true;
        }
      })();
  ...
  return (
    <MainLayout>
      <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-red-900/20 relative">
        {/* Header Banner */}
        <BannerDisplay type="header" pageLocation="casino" className="mb-4" />
        
        {/* Slider Banner */}
        <div className="w-full mb-6">
          <BannerDisplay type="slider" pageLocation="casino" className="mb-4" />
        </div>
        ...
        {/* Background effects */}
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_#dc2626_0%,_transparent_50%)] opacity-10"></div>
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_bottom_left,_#b91c1c_0%,_transparent_50%)] opacity-5"></div>
        
        {/* Hero Banner */}
        <div className="relative bg-gradient-to-r from-black via-gray-900 to-red-900/40 overflow-hidden">
        ...
        {!isLoading && uniqueProviders.length > 0 && (
          <div className="bg-gradient-to-r from-black via-red-900/10 to-black border-b border-red-500/20 py-3">
            <div className="container mx-auto px-4">
              <div className="flex items-center justify-between gap-2 mb-3">
                <span className="text-red-400 text-sm font-medium">{translate('filters.providers', 'Sağlayıcılar')} ({uniqueProviders.length}):</span>
                {uniqueProviders.length > 19 && (
                  <button
                    type="button"
                    onClick={() => setShowAllProviders(!showAllProviders)}
                    className="px-3 py-1 text-xs bg-red-500/20 hover:bg-red-500/30 text-red-300 rounded-full transition-colors"
                  >
              ...
              <div className="relative">
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all duration-200 whitespace-nowrap flex-shrink-0 ${
                        selectedProvider === 'all'
                          ...
                          : 'bg-gray-800/50 text-red-200/80 hover:bg-red-900/30 border border-red-500/20'
                      }`}
                    >
                      ...
                      return (
                        ...
                        >
                          <span className="flex items-center gap-1">
                            {provider}
                            <span className="text-xs text-red-300/60">({providerGameCount})</span>
                          </span>
                        </button>
                      );
                    })}
                ...
                {showAllProviders && uniqueProviders.length > 19 && (
                  <div className="space-y-2 mt-2 border-t border-red-500/20 pt-2">
                            ...
                            return (
                              ...
                              >
                                <span className="flex items-center gap-1.5">
                                  <span>{provider}</span>
                                  <span className="text-red-400/60">({providerGameCount})</span>
                                </span>
                              </button>
                            );
                          })}
        ...
        {/* Header Controls */}
        <div className="relative bg-gradient-to-r from-black via-gray-900 to-red-900/30 border-b border-red-500/20 backdrop-blur-sm overflow-hidden">
          <div className="container mx-auto px-4 py-6">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
              <div>
                <h2 className="text-2xl font-bold text-red-100 mb-2">
                  {selectedProvider === 'all' ? translate('gameType.all', 'Tümü') + ' Casino Oyunları' : `${selectedProvider} Casino Oyunları`}
                </h2>
                <p className="text-red-200/60 text-sm">
                  {translate('casino.subtitle', 'Canlı casino ve masa oyunları')}
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-10 bg-gray-800/50 border-red-500/30 text-white placeholder-red-200/50 focus:border-red-400"
                  />
                </div>
                ...
                <Select value={gameCategory} onValueChange={(value) => setGameCategory(value as any)}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="table">Masa Oyunları</SelectItem>
                    <SelectItem value="roulette">Rulet</SelectItem>
                ...
                {/* View Mode */}
                <div className="flex gap-1 bg-gray-800/50 rounded-lg p-1 border border-red-500/30">
                  <Button
                    size="sm"
                    variant={viewMode === 'grid' ? 'default' : 'outline'}
                    onClick={() => setViewMode('grid')}
                    className="p-3 border-red-500/30"
                  >
                  ...
                  <Button
                    ...
                    className="p-3 border-red-500/30 hover:bg-red-900/30"
        ...
        <div className="relative container mx-auto px-4 py-6">
          {isLoading ? (
            ...
            <>
              {/* Statistics Bar */}
              <div className="flex items-center justify-between mb-6 p-4 bg-gradient-to-r from-gray-900/50 via-black/30 to-gray-900/50 rounded-lg border border-red-500/20">
                <div className="flex items-center gap-6 text-sm text-red-200/80">
                  <span className="flex items-center gap-2">
                    <Eye className="w-4 h-4 text-red-400" />
                    {translate('slots.stats.showing')}: <strong className="text-red-300">{currentPageGames.length}</strong> / <strong className="text-white">{totalFilteredGames}</strong>
                  </span>
                  <span className="flex items-center gap-2">
                  ...
                  {gameCategory !== 'all' && (
                    <span className="flex items-center gap-2 px-3 py-1 bg-red-500/20 border border-red-500/30 rounded-full">
                      <Filter className="w-4 h-4 text-red-400" />
                      <span className="text-red-200 font-medium">
                        {gameCategory === 'live' ? 'Canlı Casino' :
                         gameCategory === 'table' ? 'Masa Oyunları' :
                         gameCategory === 'roulette' ? 'Rulet' :
                         gameCategory === 'blackjack' ? 'Blackjack' :
                         gameCategory === 'baccarat' ? 'Baccarat' :
