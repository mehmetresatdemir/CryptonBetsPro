  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (user) {
        ...
        if (user.tckn) {
          autoFilledData.tc_number = user.tckn;
        }
  ...
  useEffect(() => {
    if (paymentMethodsError) {
      ...
      toast({
        ...
        description: (paymentMethodsError as Error).message || 'Ödeme yöntemleri yüklenemedi',
        variant: 'destructive'
      });
    }
  ...
  const handleDepositRequest = async () => {
    ...
    try {
      ...
      const userInfo = {
        ...
        pay_co_full_name: `${user.firstName || user.username || 'Kullanıcı'} ${user.lastName || ''}`.trim(),
        full_name: `${user.firstName || user.username || 'Kullanıcı'} ${user.lastName || ''}`.trim(),
        fullName: `${user.firstName || user.username || 'Kullanıcı'} ${user.lastName || ''}`.trim()
      };
      ...
      switch (selectedMethod.id) {
        ...
        case 'pep':
          ...
          autoGeneratedData.tc_number = user.tckn || '12345678901'; // Varsayılan TC
          break;
        case 'paratim':
        ...
        case 'payco':
          ...
          autoGeneratedData.pay_co_full_name = `${user.firstName || user.username || 'Kullanıcı'} ${user.lastName || ''}`.trim();
          break;
        case 'payfix':
        ...
        case 'kredikarti':
          ...
          autoGeneratedData.tc_number = user.tckn || '12345678901';
          break;
        case 'havale':
          ...
          autoGeneratedData.account_name = `${user.firstName || user.username || 'Kullanıcı'} ${user.lastName || ''}`.trim();
          break;
      }
      ...
      const depositData = {
        ...
        tc_number: (selectedMethod.id === 'kredikarti') 
          ? (user.tckn || autoGeneratedData.tc_number) 
